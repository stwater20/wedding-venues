...
網站：<https://stwater20.github.io/wedding-venues/>
...
在 `data.js` 的 `VENUES` 陣列裡照下方「資料格式規範」加一個物件，commit 到 `main`，網站約 1 分鐘後自動更新。不是協作者的話，歡迎 fork 後發 Pull Request。
...
2. 自己的裝置：網站右上「同步設定」→ 貼上 token（密語選填）→ 儲存並測試
3. 按「📋 複製給另一半的連結」，把複製好的連結**私下**傳給另一半，她點開就自動完成設定，顯示「✅ 同步已開啟」

連結長這樣（含 token 與密語，切勿公開張貼）：

```text
https://stwater20.github.io/wedding-venues/#setup=<token>&pass=<密語>
```

### 產生 token 的步驟

1. 開 <https://github.com/settings/personal-access-tokens/new>（GitHub 可能先要求手機 App 或 Email 驗證身分，通過後才會看到表單）
2. **Repository access** 選 **Only select repositories** → 只勾 `wedding-venues`
3. **Permissions** → 按 **Add permissions** → 搜尋並勾選 **Contents** → 把它的 Access 設為 **Read and write**（其他權限都不用加）
4. 按 **Generate token**，複製 token → 回網站右上「同步設定」→ 貼上 → 儲存並測試

### 備註加密（選填）

「同步設定」裡可填一組**密語**（兩人要填同一組）。填了之後，備註存到 `state.json` 前會先在裝置上用 AES-GCM 加密，雲端只看得到亂碼，只有輸入相同密語的裝置能還原。收藏與狀態不加密（不敏感）。密語忘了會解不開舊備註，請挑好記的。「複製給另一半的連結」會自動把密語帶進去，一次設定完成。

### 注意事項

- Token 只存在該裝置的瀏覽器 localStorage，不會出現在網頁原始碼或 repo 裡
...
- 另一半不需要 GitHub 帳號也能「看」（網站是公開的）；要「寫」才需要 token
...
所有價格與方案內容為官網及公開媒體（新娘物語 [weddings.tw](https://www.weddings.tw/) 等）資訊的整理，僅供個人比較參考，非商業用途，正式價格以各飯店報價為準。照片採直連原站方式呈現，版權屬原網站所有。
